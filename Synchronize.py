...
def clip_normalize(
    signal, fps, signal_time, signal_range, img_counter, img_time, type="ECG"
):
        ...
        if have_signal:
            if type == "ECG" or type == "PPG":
                ...
                if delete_signal >= 0:
                    ...
                    # print(' time=',float(t_img_time)/10,' input_signal=',input_signal.shape[0],' img_counter=',img_counter[i],'target len=',target_len,' delete=',delete_signal)
                    n_signal = np.interp(
                        np.linspace(1, input_signal.shape[0], target_len),
                ...
                if delete_signal >= 0:
                    ...
                    n_signal = np.interp(
                        np.linspace(1, input_signal.shape[0], img_counter[i]),
                        np.linspace(1, input_signal.shape[0], input_signal.shape[0]),
                        input_signal,
                    )
                    # print('resample shape=',n_signal.shape[0])
...
def normalize(root, file_dir):
    ...
    with open(record_info, "r") as f:
        ...
        for time in str1[1].split():
            ...
            if counter == 0:
                ...
                last = int(
                    float(x) * 10
                )  # record last time 1.1s 1.1s 1.1s => 11s 11s 11s
                time_counter = 0
    ...
    # normalize
    re_ecg, delete_img, resample_len_ecg, save_time, img_len = clip_normalize(
        ecg, 1000, ppg_time, ecg_counter, img_counter, img_time, "ECG"
    )
    re_ppgr, delete_img, resample_len_ppgr, save_time, img_len = clip_normalize(
        ppgr, 100, ppg_time, ppg_counter, img_counter, img_time, "rPPG"
    )
    re_ppgir, delete_img, resample_len_ppgir, save_time, img_len = clip_normalize(
        ppgir, 100, ppg_time, ppg_counter, img_counter, img_time, "rPPG"
    )
    ...
    for i in img_list:
        ...
        os.rename(
            os.path.join(path, i), name
        )  # 改名時，使用字串格式化的方式進行三位數補零
        n = n + 1  # 每次重複時將 n 增加 1

    time_step = open(os.path.join(normalize_dir, "syn_time_step.txt"), "w")
    for i in range(len(img_len)):
        time_step.write(
            f"{save_time[i]} {resample_len_ecg[i]} {resample_len_ppgir[i]} {resample_len_ppgr[i]} {img_len[i]}\n"
        )
    ...
    information.write(
        f"original ECG length    = {ecg_len} normalize length = {re_ecg.shape[0]}\n"
    )
    information.write(
        f"original PPG_R length  =  {ppgr_len} normalize length = {re_ppgr.shape[0]}\n"
    )
    information.write(
        f"original PPG_IR length =  {ppgir_len} normalize length = {re_ppgir.shape[0]}\n"
    )
    information.write(
        f"original image length  =  {len(img)} normalize length = {len(img_list)}\n"
    )
