...
def FLOW_CTRL_BTN_action():
    ...
    if FLOW_CTRL_STATE == 1:  # Try to connect to device
        ...
        threading.Thread(target= turn_on_camera).start() # turn on camera
        ...
        if FLAG_ALLOW_CONN:
            try:
                ...
                if conn_connected:
                    ...
                    #waveform_draw_task() #original draw waveform
...
def waveform_draw_task():
    ...
    #    frm_fig.after(waveform_draw_period, waveform_draw_task) #一直循環call function 讓視頻閃爍
...
def turn_on_camera():
    ...
    while(FLAG_SENSING_EN):
        ...
        if ipcam.status:
            try:
                frame = cv2.resize(frame, (image_width, image_height), interpolation=cv2.INTER_AREA) #有可能第一張收不到frame
            except:
            ...
            #add rectangle
            cv2.rectangle(frame, (int(5*frame.shape[1]/10+frame.shape[1]/20), int(frame.shape[0]/5)), (int(9*frame.shape[1]/10), int(4*frame.shape[0]/5)), (0, 0, 255), 2)
            cv2.rectangle(frame, (int(1*frame.shape[1]/10), int(1*frame.shape[0]/6)), (int(5*frame.shape[1]/10), int(5*frame.shape[0]/6)), (0, 0, 255), 2)
            # show frame on tk
            ...
            #canvas_v.delete("all") 
            #canvas_v.create_image(0,0,anchor=NW,image=imgtk)
            #canvas_v.image=imgtk
            ...
            while(Btn_Record_State ):
                ...
                cv2.putText(frame, text, (x, y + text_h + font_scale - 1), font, font_scale, (255, 255, 255), font_thickness)
                #放框框
                cv2.rectangle(frame, (int(5*frame.shape[1]/10+frame.shape[1]/20), int(frame.shape[0]/5)), (int(9*frame.shape[1]/10), int(4*frame.shape[0]/5)), (0, 0, 255), 2)
                cv2.rectangle(frame, (int(1*frame.shape[1]/10), int(1*frame.shape[0]/6)), (int(5*frame.shape[1]/10), int(5*frame.shape[0]/6)), (0, 0, 255), 2)
                test=time.time()
                ...
                movieLabel.image=imgtk
                out_time = time.time()
                #print(f'total time = {out_time-now_time}')
                #print(f'tk time = {out_time-test}')
...
if __name__ == "__main__":
    ...
    font=('Arial',15,'bold')  #字體 #width 跟 字體都有放大效果，不過要注意，如果因文字過長，物件的width不夠，會導致字無法顯示的情形喔
    figsize=(9,6)  #繪圖大小
    # BLE block
    ...
    mat_fig, mat_ax = plt.subplots(3, 1, figsize=figsize, dpi=100, constrained_layout=True)
