...
class CameraControlApp:
    def __init__(self, root):
        ...
        self.canvas.get_tk_widget().grid(
            row=0, column=2, rowspan=3, padx=10, pady=5
        )  # 使用grid()方法放置在网格中

        # 创建按钮
        self.apply_button = ttk.Button(
            self.root, text="應用", command=self.apply_settings
        )
        self.apply_button.grid(
            row=3, column=0, columnspan=2, padx=10, pady=5, sticky=tk.W + tk.E
        )

        self.record_button = ttk.Button(
            self.root, text="Record", command=self.record_toggle
        )
        self.record_button.grid(
            row=4, column=0, columnspan=2, padx=10, pady=5, sticky=tk.W + tk.E
        )
    ...
    def show_camera_feed(self):
        ...
        if ret:
            ...
            photo = ImageTk.PhotoImage(
                image=Image.fromarray(frame_with_timestamp), master=self.root
            )
    ...
    def playback_recorded_frames(self):
        ...
        for i in range(frame.shape[0]):
            ...
            if np.issubdtype(combined_frame.dtype, np.floating):
                combined_frame = np.clip(
                    combined_frame, 0, 1
                )  # Clip float pixel values to [0, 1]
            elif np.issubdtype(combined_frame.dtype, np.integer):
                combined_frame = np.clip(
                    combined_frame, 0, 255
                )  # Clip integer pixel values to [0, 255]
            else:
